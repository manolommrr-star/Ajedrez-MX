 ...
 *
 * TXT de importación (File → Import players from text file):
 *   - Separador TABULACIÓN · finales CRLF · codificación Windows-1252.
 * Solo exporta estados jugables (pagada/confirmada/checkin) salvo
 * `{ incluirTodos: true }`. Formato PROPUESTO hasta verificarse con el
 * asistente real (guía en docs/04-swiss-manager.md).
 *
...
const CODIGOS_FEDERACION = new Map([['FENAMAC', 'MEX'], ['MEXICO', 'MEX']]);
...
/** Convierte participantes a filas TXT + advertencias de calidad. */
function aFilasTxt(participantes) {
  ...
  for (const p of participantes) {
    const j = p.jugador || {};
    filas.push([
      limpiarTxt(j.apellidos), limpiarTxt(j.nombre), federacionSwiss(j.federacion),
      limpiarTxt(j.fideId), eloSwiss(j.elo), tituloSwiss(j.titulo), sexoSwiss(j.sexo),
      fechaSwiss(j.fechaNacimiento), limpiarTxt(j.club), limpiarTxt(j.ciudad),
